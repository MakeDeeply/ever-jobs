...
import {
  FRAMEWORK_APPLY_URL,
  FRAMEWORK_CAREERS_URL,
  FRAMEWORK_COMPANY_NAME,
  FRAMEWORK_DEFAULT_RESULTS,
  FRAMEWORK_DEFAULT_TIMEOUT_SECONDS,
  FRAMEWORK_JD_SECTION_NAMES,
  FRAMEWORK_LOCATION_FRAMER_NAME,
  FRAMEWORK_ORIGIN,
  FRAMEWORK_ROLE_PATH,
  FRAMEWORK_SALARY_FRAMER_NAME,
} from './framework.constants';
import { FrameworkDetail, FrameworkOpening } from './framework.types';
...
@Injectable()
export class FrameworkService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    try {
      const client = createHttpClient({
        ...
        caCert: input.caCert,
        requestTimeout: input.requestTimeout ?? FRAMEWORK_DEFAULT_TIMEOUT_SECONDS,
      });
  ...
  protected async fetchListingHtml(
    ...
    const res = await client.get<string>(FRAMEWORK_CAREERS_URL);
   ...
   */
  private parseListing(html: string): FrameworkOpening[] {
    const $ = cheerio.load(html);
    const seen = new Set<string>();
    const openings: FrameworkOpening[] = [];

    $(`a[href*="${FRAMEWORK_ROLE_PATH}"]`).each((_i, el) => {
      ...
      openings.push({
        slug,
        detailUrl: this.absoluteUrl(`${FRAMEWORK_ROLE_PATH}${slug}`),
   ...
   * rich-text sections (see FRAMEWORK_JD_SECTION_NAMES).
   */
  private parseDetail(html: string): FrameworkDetail {
    const $ = cheerio.load(html);

    const title = this.titleFromDocTitle($);
    const location = this.framerNamedText($, FRAMEWORK_LOCATION_FRAMER_NAME);
    const salaryText = this.framerNamedText($, FRAMEWORK_SALARY_FRAMER_NAME);
  ...
  private toJobPost(
    ...
    return new JobPostDto({
      id: `framework-${opening.slug}`,
      site: Site.FRAMEWORK,
      title,
      companyName: FRAMEWORK_COMPANY_NAME,
      companyUrl: FRAMEWORK_CAREERS_URL,
  ...
  private descriptionFromSections($: cheerio.CheerioAPI): string | null {
    const wanted = new Set(FRAMEWORK_JD_SECTION_NAMES);
    const parts: string[] = [];
  ...
  private slugFromHref(href: string): string {
    ...
    const idx = path.indexOf(FRAMEWORK_ROLE_PATH);
    const tail = idx >= 0 ? path.slice(idx + FRAMEWORK_ROLE_PATH.length) : '';
    return tail.replace(/^\/+|\/+$/g, '').toLowerCase();
  }

  private absoluteUrl(href: string): string {
    try {
      return new URL(href, FRAMEWORK_ORIGIN).toString();
    } catch {
      return href;
    }
  }
