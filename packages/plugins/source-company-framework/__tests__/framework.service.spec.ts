...
import { FrameworkModule } from '../src/framework.module';
import { FrameworkService } from '../src/framework.service';

const FIXTURES = join(__dirname, 'fixtures');
const read = (name: string): string =>
  readFileSync(join(FIXTURES, `${name}.html`), 'utf8');

const LISTING = read('framework-hiring');
const DETAIL: Record<string, string> = {
  'senior-software-engineer': read('framework-senior-software-engineer'),
  'senior-mechanical-engineer': read('framework-senior-mechanical-engineer'),
};
 ...
 */
function makeService(listing = LISTING): FrameworkService {
...
describe('FrameworkService (Spec 5063 — Framer two-step careers)', () => {
  it('resolves through FrameworkModule via NestJS DI', async () => {
    const moduleRef = await Test.createTestingModule({
      imports: [FrameworkModule],
    }).compile();
    expect(moduleRef.get(FrameworkService)).toBeInstanceOf(FrameworkService);
  });

  it('exports the Site.FRAMEWORK = "framework" enum value', () => {
  ...
  it('enumerates both live roles with on-domain jobUrl + shared /apply, empty fields empty', async () => {
    ...
    const swe = byId.get('framework-senior-software-engineer');
    expect(swe).toBeTruthy();
    expect(swe?.site).toBe(Site.FRAMEWORK);
  ...
  it('carries the detail JD sections into the description as markdown', async () => {
    ...
    const swe = jobs.find(
      (j) => j.id === 'framework-senior-software-engineer',
    );
