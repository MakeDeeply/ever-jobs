...
import {
  ...
  HYLIO_CAREERS_URL,
  HYLIO_COMPANY_NAME,
  HYLIO_DEFAULT_RESULTS,
  HYLIO_DEFAULT_TIMEOUT_SECONDS,
  HYLIO_ORIGIN,
  HYLIO_ROLE_PATH,
} from './hylio.constants';
import { HylioDetail, HylioOpening } from './hylio.types';
...
export class HylioService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    try {
      const client = createHttpClient({
        ...
        caCert: input.caCert,
        requestTimeout: input.requestTimeout ?? HYLIO_DEFAULT_TIMEOUT_SECONDS,
      });
  ...
  protected async fetchListingHtml(
    ...
    const res = await client.get<string>(HYLIO_CAREERS_URL);
  ...
  private parseListing(html: string): HylioOpening[] {
    ...
    $('.jobtitle').each((_i, el) => {
      ...
      scope.find(`a[href*="${HYLIO_ROLE_PATH}"]`).each((_j, a) => {
        if (detailHref) return;
        const href = $(a).attr('href') ?? '';
        const s = this.slugFromHref(href);
        if (s && s !== HYLIO_BOARD_SLUG) detailHref = href;
      });
      if (!detailHref) return;
  ...
  private toJobPost(opening: HylioOpening, detail: HylioDetail): JobPostDto {
    ...
    // detail page.
    const jobUrl = opening.detailUrl ?? opening.applyUrl ?? HYLIO_CAREERS_URL;

    return new JobPostDto({
      id: `hylio-${opening.slug}`,
      site: Site.HYLIO,
      title,
      companyName: HYLIO_COMPANY_NAME,
      companyUrl: HYLIO_CAREERS_URL,
      jobUrl,
  ...
  private slugFromHref(href: string): string {
    ...
    const idx = path.indexOf(HYLIO_ROLE_PATH);
    const tail = idx >= 0 ? path.slice(idx + HYLIO_ROLE_PATH.length) : '';
    return tail.replace(/\/+$/, '').toLowerCase();
  }

  private absoluteUrl(href: string): string {
    try {
      return new URL(href, HYLIO_ORIGIN).toString();
    } catch {
      return href;
    }
  }
