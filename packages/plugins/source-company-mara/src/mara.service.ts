...
import {
  ...
  MARA_CAREERS_URL,
  MARA_COMPANY_NAME,
  MARA_DEFAULT_RESULTS,
  MARA_DEFAULT_TIMEOUT_SECONDS,
  MARA_HIGHLIGHT_SELECTOR,
  MARA_LABEL_SELECTOR,
  MARA_TITLE_SELECTOR,
} from './mara.constants';
import { MaraOpening } from './mara.types';
...
@Injectable()
export class MaraService implements IScraper {
  private readonly logger = new Logger(MaraService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    try {
      const client = createHttpClient({
        ...
        caCert: input.caCert,
        requestTimeout: input.requestTimeout ?? MARA_DEFAULT_TIMEOUT_SECONDS,
      });
  ...
  protected async fetchCareersHtml(
    ...
    const res = await client.get<string>(MARA_CAREERS_URL);
   ...
   */
  private parseCareers(html: string): MaraOpening[] {
    const $ = cheerio.load(html);
    const seen = new Set<string>();
    const openings: MaraOpening[] = [];

    $(MARA_CARD_SELECTOR).each((_i, el) => {
      const card = $(el);

      const applyUrl = card.find(MARA_APPLY_SELECTOR).first().attr('href');
      if (!applyUrl) return;
  ...
  private buildTitle(
    ...
    const large = this.normalize(card.find(MARA_TITLE_SELECTOR).first().text());
    if (!large) return '';

    const small = this.normalize(
      card.find(MARA_HIGHLIGHT_SELECTOR).first().text(),
    );
  ...
  private labelFields(
    $: cheerio.CheerioAPI,
    card: cheerio.Cheerio<AnyNode>,
  ): Pick<MaraOpening, 'location' | 'employmentType'> {
    let location: string | null = null;
    let employmentType: string | null = null;

    card.find(MARA_LABEL_SELECTOR).each((_i, el) => {
  ...
  private toJobPost(opening: MaraOpening): JobPostDto {
    ...
    return new JobPostDto({
      ...
      site: Site.MARA,
      title: opening.title,
      companyName: MARA_COMPANY_NAME,
      companyUrl: MARA_CAREERS_URL,
