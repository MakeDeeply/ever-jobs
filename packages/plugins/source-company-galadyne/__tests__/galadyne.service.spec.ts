...
import { GaladyneService } from '../src/galadyne.service';
...
function serviceWith(
  ...
  const service = new GaladyneService();
...
describe('GaladyneService', () => {
  ...
  it('maps identity, stated location, and on-page apply page', async () => {
    ...
    expect(prop.site).toBe(Site.GALADYNE);
    expect(prop.id).toBe('galadyne-propulsion-engineer');
  ...
  it('builds the JD description from the client chunk content', async () => {
    ...
    expect(tech.id).toBe('galadyne-integration-technician-all-levels');
  });
