...
import {
  GALADYNE_CAREERS_URL,
  GALADYNE_CHUNK_HREF_RE,
  GALADYNE_COMPANY_NAME,
  GALADYNE_DEFAULT_RESULTS,
  GALADYNE_DEFAULT_TIMEOUT_SECONDS,
  GALADYNE_ORIGIN,
} from './galadyne.constants';
import { GaladyneCard, GaladyneContent } from './galadyne.types';
...
@Injectable()
export class GaladyneService implements IScraper {
  private readonly logger = new Logger(GaladyneService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    try {
      const client = createHttpClient({
        ...
        caCert: input.caCert,
        requestTimeout: input.requestTimeout ?? GALADYNE_DEFAULT_TIMEOUT_SECONDS,
      });

      const listingHtml = await this.fetchText(client, GALADYNE_CAREERS_URL);
   ...
   */
  private parseCards(html: string): GaladyneCard[] {
    const $ = cheerio.load(html);
    const cards: GaladyneCard[] = [];
  ...
  private async fetchContent(
    client: ReturnType<typeof createHttpClient>,
    listingHtml: string,
  ): Promise<Map<string, GaladyneContent>> {
    const match = GALADYNE_CHUNK_HREF_RE.exec(listingHtml);
    if (!match) {
      this.logger.warn('Galadyne: careers chunk URL not found');
      return new Map();
    }
    try {
      const chunk = await this.fetchText(client, `${GALADYNE_ORIGIN}${match[0]}`);
   ...
   */
  private parseContent(chunk: string): Map<string, GaladyneContent> {
    const out = new Map<string, GaladyneContent>();
  ...
  private toJobPost(
    card: GaladyneCard,
    content: GaladyneContent | undefined,
  ): JobPostDto {
    return new JobPostDto({
      id: `galadyne-${this.slugify(card.title)}`,
      site: Site.GALADYNE,
      title: card.title,
      companyName: GALADYNE_COMPANY_NAME,
      companyUrl: GALADYNE_CAREERS_URL,
      jobUrl: GALADYNE_CAREERS_URL,
      applyUrl: GALADYNE_CAREERS_URL,
  ...
  private description(content: GaladyneContent | undefined): string | null {
