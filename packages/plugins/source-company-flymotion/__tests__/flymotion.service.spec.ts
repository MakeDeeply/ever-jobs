 ...
 */
function makeService(listing = LISTING): FlymotionService {
  const svc = new FlymotionService();
...
describe('FlymotionService', () => {
  it('enumerates every live role with an on-page apply URL', async () => {
    ...
    const job = jobs[0];
    expect(job.site).toBe(Site.FLYMOTION);
    expect(job.companyName).toBe('FLYMOTION');
    expect(job.id).toBe('flymotion-event-coordinator');
