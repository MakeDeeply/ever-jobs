...
import { VightModule } from '../src/vight.module';
import { VightService } from '../src/vight.service';

const FIXTURES = join(__dirname, 'fixtures');
const read = (name: string): string =>
  readFileSync(join(FIXTURES, `${name}.html`), 'utf8');

const LISTING = read('vight-join-us');
const DETAIL: Record<string, string> = {
  gnc: read('vight-gnc'),
  propulsion: read('vight-propulsion'),
  'chief-engineer': read('vight-chief-engineer'),
};
 ...
 */
function makeService(listing = LISTING): VightService {
  const svc = new VightService();
...
describe('VightService (Spec 5066 — hand-coded static two-step careers)', () => {
  it('resolves through VightModule via NestJS DI', async () => {
    const moduleRef = await Test.createTestingModule({
      imports: [VightModule],
    }).compile();
    expect(moduleRef.get(VightService)).toBeInstanceOf(VightService);
  });

  it('exports the Site.VIGHT = "vight" enum value', () => {
    expect(Site.VIGHT).toBe('vight');
  });
  ...
  it('enumerates the 3 real roles + the generalist card', async () => {
    ...
    expect(new Set(jobs.map((j) => j.id))).toEqual(
      new Set([
        'vight-gnc',
        'vight-propulsion',
        'vight-chief-engineer',
        'vight-exceptional-generalist',
      ]),
    );

    for (const job of jobs) {
      expect(job.site).toBe(Site.VIGHT);
  ...
  it('uses the detail-page title/location/type + jobUrl for real roles', async () => {
    ...
    for (const id of ['vight-gnc', 'vight-propulsion', 'vight-chief-engineer']) {
  ...
  it('carries all detail JD sections (incl. About Vight boilerplate) as markdown', async () => {
    const svc = makeService();
    const { jobs } = await svc.scrape(new ScraperInputDto());
    const gnc = jobs.find((j) => j.id === 'vight-gnc');
  ...
  it('keeps the generalist from the card alone (no detail, no location/type)', async () => {
    const svc = makeService();
    const { jobs } = await svc.scrape(new ScraperInputDto());
    const gen = jobs.find((j) => j.id === 'vight-exceptional-generalist');
  ...
  it('applies searchTerm / offset / resultsWanted filters', async () => {
    ...
    expect(match.jobs[0].id).toBe('vight-gnc');
