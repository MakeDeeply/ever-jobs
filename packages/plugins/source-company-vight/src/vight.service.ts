...
import {
  ...
  VIGHT_CF_EMAIL_PATH,
  VIGHT_COMPANY_NAME,
  VIGHT_DEFAULT_RESULTS,
  VIGHT_DEFAULT_TIMEOUT_SECONDS,
  VIGHT_ORIGIN,
} from './vight.constants';
import { VightDetail, VightOpening } from './vight.types';
...
@Injectable()
export class VightService implements IScraper {
  private readonly logger = new Logger(VightService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    try {
      const client = createHttpClient({
        ...
        caCert: input.caCert,
        requestTimeout: input.requestTimeout ?? VIGHT_DEFAULT_TIMEOUT_SECONDS,
      });
      ...
      const settled = await Promise.allSettled(
        openings.map(async (opening) => {
          let detail: VightDetail | null = null;
  ...
  protected async fetchListingHtml(
    ...
    const res = await client.get<string>(VIGHT_CAREERS_URL);
   ...
   */
  private parseListing(html: string): VightOpening[] {
    const $ = cheerio.load(html);
    const openings: VightOpening[] = [];
    ...
    $('article.role').each((_i, el) => {
      ...
      const isEmailApply =
        ...
        applyHref.includes(VIGHT_CF_EMAIL_PATH);
      const detailUrl =
   ...
   */
  private parseDetail(html: string): VightDetail {
    const $ = cheerio.load(html);
  ...
  private toJobPost(
    opening: VightOpening,
    detail: VightDetail | null,
  ): JobPostDto {
    ...
    return new JobPostDto({
      id: `vight-${opening.slug}`,
      site: Site.VIGHT,
      title,
      companyName: VIGHT_COMPANY_NAME,
      companyUrl: VIGHT_CAREERS_URL,
      // Canonical URL is the employer's own detail page; the generalist has none.
      jobUrl: opening.detailUrl ?? VIGHT_CAREERS_URL,
      location,
      description,
      isRemote: false,
  ...
  private decodeCfEmail(href: string): string | null {
    ...
    if (!href.includes(VIGHT_CF_EMAIL_PATH) || hashIdx < 0) return null;
  ...
  private absoluteUrl(href: string): string {
    try {
      return new URL(href, VIGHT_CAREERS_URL).toString();
    } catch {
      try {
        return new URL(href, VIGHT_ORIGIN).toString();
      } catch {
        return href;
      }
    }
