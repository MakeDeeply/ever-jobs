...
| Related specs | 5042/5045/5046/5047/5048/5049/5050/5051/5052/5053/5056/5057 (prior company careers plugins) |
...
- Map each card to `JobPostDto`:
    ...
    - `location` — the per-role stated location from the title suffix (e.g. `Virginia`) via `parseLocationList`; null when a role has no suffix
...
- **No HQ substitution.** Location is only what each role states (`Virginia`); never the `Charlotte, NC` corporate HQ from external data.
...
- per-role stated location surfaces as `Virginia`; `isRemote=false`
